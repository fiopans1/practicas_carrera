...
struct total{//struct para el hilo que calcula el total del banco
    struct bank *bank;
    int cnt;
...
//funcion auxiliar para el while de calcular total banco
...
void *calcular_totales(void *ptr){//tenemos que bloquear el array de mutex todos para hacer el for
    struct total *tt= ptr;
    int total=0;
    while(comprobar(&tt->cnt,&tt->mutex)){//bloqueamos todo, sumamos, y desbloqueamos
...
void wait_transferencias(struct options opt, struct bank *bank, struct thread_info *threads, struct total *tt) {
    ...
    pthread_mutex_lock(&tt->mutex);//seccion critica para parar el mutex de total
    tt->cnt = 0;
...
void init_accounts(struct bank *bank, int num_accounts) {
    //inicializa un array con num_accounts posiciones, y cada una representa una cuenta de banco
...
int main (int argc, char **argv)
{
    ...
    struct total *tt;//contiene informacion para el thread de total banco
    srand(time(NULL));
    ...
    tt = init_total(&bank);//crea hilo para ejecutar lo de calcular total banco
