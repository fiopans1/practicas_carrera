...
struct thread_info *start_threads(struct options opt, struct bank *bank)//le paso valores por defecto y el banco
{
    int i;
...
void wait(struct options opt, struct bank *bank, struct thread_info *threads) {// Wait for the threads to finish
...
int main (int argc, char **argv)
{
    ...
    read_options(argc, argv, &opt);//iguala los valores a las opciones que tenemos por defecto

    init_accounts(&bank, opt.num_accounts);//inicializa las cuentas

    thrs = start_threads(opt, &bank);//crea los hilos y hace que ejecute desposito
