...
let rec typeof ctx tm =
  ...
  | TmIsNil (ty,t) ->
        ...
        (* else raise (Type_error ("argument of isnil is not a " ^ (pretty_print_ty ctx ty) ^ " list")) *)

      (* T-Head *)
  | TmHead (ty,t) ->
        if tyarrowtolarge(typeof ctx t) = (tyarrowtolarge(TyList(obtTy(ty)))) then obtTy(ty)
        else raise (Type_error ("argument of isnil is not a type list"))
        (* else raise (Type_error ("argument of head is not a " ^ (pretty_print_ty ctx ty) ^ " list")) *)

      (* T-Tail *)
  | TmTail (ty,t) ->
        if tyarrowtolarge(typeof ctx t) = (tyarrowtolarge(TyList(obtTy(ty)))) then TyList(obtTy(ty))
        else raise (Type_error ("argument of isnil is not a type list"))
        (* else raise (Type_error ("argument of tail is not a " ^ (pretty_print_ty ctx ty) ^ " list")) *)
